...
def books(request):
    try:
        ...
        # DB tables missing (SQLite fallback). Show empty books list for testing.
        form = SearchBookForm()
        context = {'form': form, 'books': []}
        return render(request, 'books.html', context)
...
def book(request, book_isbn):
    try:
        ...
        # force evaluation so DB errors are caught here rather than in templates
        try:
            comments_qs = models.Comments.objects.filter(isbn=book.isbn)
            comments = list(comments_qs)
        except OperationalError:
            comments = []
    except OperationalError:
        # If legacy DB tables aren't present (SQLite fallback), show degraded page
        book = None
        storages = []
        comments = []
    all=[]
...
def loans(request):
    ...
    try:
        if request.user.is_staff:
            loans_qs = models.Loans.objects.all()
        else:
            loans_qs = models.Loans.objects.filter(id=request.user.id)
        ...
        # Database tables for the legacy `library` app are not present (for
        # example when using the SQLite fallback). Return empty list instead
        # of raising a 500 so the site remains usable for testing.
        all = []
...
def comments(request):
    # Allow client to choose order via ?order=asc or ?order=desc (default desc)
    order = request.GET.get('order', 'desc').lower()
    try:
        if request.user.is_staff:
            qs = models.Comments.objects.all()
        else:
            qs = models.Comments.objects.filter(id=request.user.id)

        if order == 'asc':
            ordered = qs.order_by('comno')
        else:
            ordered = qs.order_by('-comno')

        # force evaluation here so any DB OperationalError is raised inside
        # this try/except and we can return a safe empty list for templates.
        comments = list(ordered)
    except OperationalError:
        # When using the SQLite fallback or the MySQL schema isn't available,
        # return an empty list instead of raising a 500 so the UI remains usable.
        comments = []

    context = {'comments': comments, 'order': order}
...
def reserves(request):
    try:
        if request.user.is_staff:
            qs = models.Reserves.objects.all()
        else:
            qs = models.Reserves.objects.filter(id=request.user.id)

        # force evaluation so DB errors are caught here rather than in template
        reserves = list(qs)
    except OperationalError:
        reserves = []
